...
class addTransaction(ttk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        self.title_Label = ttk.Label(self,text="Add Transaction",font=("Bodoni MT",24,"bold"),background="#333333",foreground="white",anchor="center",padding="40")
    ...
    def Submit(self):
        if self.amount.get().strip()!="" and self.date.get()!="":
            try :
                tokens = self.parent.master.load_tokens()
                userId = tokens["id"]
                access_token = tokens["access_token"]
                refresh_token = tokens["refresh_token"]
                response = requests.post(f"{API_URL}transaction",headers={"Authorization": f"Bearer {access_token}"},
                                        json={"userId":userId,"amount":int(self.amount.get().strip()),
